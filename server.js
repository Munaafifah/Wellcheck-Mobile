...
app.post("/appointments", async (req, res) => {
  try {
    ...
    const {
      appointmentDate, appointmentTime, duration, typeOfSickness,
      additionalNotes, email, insuranceProvider, insurancePolicyNumber,
      hospitalId, registeredHospital,
    ...
    const newAppointment = {
      appointmentId, userId, doctorId, hospitalId,
      appointmentDate, appointmentTime, duration, registeredHospital,
      typeOfSickness, additionalNotes: additionalNotes || null,
      insuranceProvider: insuranceProvider || null,
      insurancePolicyNumber: insurancePolicyNumber || null,
      email, costItems: [], drugCost: 0,
      statusPayment, statusAppointment,
    ...
    jwt.verify(token, secretKey, async (err, decoded) => {
      ...
      try {
        ...
        const billings = userAppointments.map((appt) => {
          ...
          const totalCost = [...drugCosts, ...costItems].reduce((sum, item) => sum + item.amount, 0);

          return {
            billingId: appt.appointmentId, userId, drugCosts, costItems, totalCost,
    ...
    jwt.verify(token, secretKey, async (err, decoded) => {
      ...
      try {
        ...
        if (!schedule.workingDays.includes(dayName)) {
          return res.json({
            doctorId, date, dayName, isWorkingDay: false, slots: [],
        ...
        for (let t = startTotal; t < endTotal; t += slotDuration) {
          ...
          if (!isDuringBreak) allSlots.push(timeStr);
        }

        const bookedAppointments = await appointments.find({
          doctorId, appointmentDate: date,
          statusAppointment: { $in: ["Approved", "Not Approved"] },
        }).toArray();
        ...
        res.json({
          doctorId, doctorName: schedule.doctorName, date, dayName,
          isWorkingDay: true, slotDurationMinutes: slotDuration, slots,
        });
      } catch (error) {
...
// Connect to MongoDB on startup then start server
getConnection()
  .then(() => {
