...
// Fetch unpaid billing data for a user
app.get('/api/billing/:userId', async (req, res) => {
  const { userId } = req.params;

  try {
    // Get unpaid appointments and prescriptions
    ...
    const totalAppointmentCost = appointments.reduce((sum, app) => sum + app.appointmentCost, 0);
    const totalPrescriptionCost = prescriptions.reduce((sum, pres) => {
      return sum + pres.medicineList.reduce((medSum, med) => medSum + med.cost, 0);
    }, 0);

    const totalCost = totalAppointmentCost + totalPrescriptionCost;

    const billingData = {
      billingId: new ObjectId().toString(),
      userId,
      appointments,
      prescriptions: prescriptions.map(p => ({
        ...p,
        totalPrescriptionCost: p.medicineList.reduce((sum, med) => sum + med.cost, 0),
      })),
      totalCost,
      statusPayment: "Not Paid",
      timestamp: new Date(),
...
app.post('/api/billing/pay', async (req, res) => {
  ...
  try {
    // Update appointments and prescriptions to "Paid"
    ...
    res.json({ message: "Billing payment completed successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
